...
#Globals
platform=${1} # (Optional) Platform,  linux or osx. Default is linux. Always builds x64 version.
...
SHARPMAKE_EXECUTABLE="$SCRIPTPATH/tmp/bin/debug/sharpmake.application/Sharpmake.Application"
SHARPMAKE_EXECUTABLE_RELEASE="$SCRIPTPATH/tmp/bin/release/sharpmake.application/Sharpmake.Application"
...
CompileSharpmake()
{
  ...
  BUILD_CMD="dotnet build \"$file\" -v q -nologo -c \"$target\" -r $platform-x64"
